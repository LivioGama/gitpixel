...
# NOTE: gemini --yolo caused hangs (agent stuck in tool-call loops); --approval-mode
# plan (read-only) works and is appropriate for a localization task. The default
# gemini model (gemini-3-flash-preview) hits a quota fallback loop; explicitly
# specifying -m gemini-2.5-flash avoids this. opencode's default model requires
# payment; google/gemini-2.5-flash works with the configured Google API key.
# codex needs --skip-git-repo-check for non-repo dirs. codex uses --sandbox
# workspace-write so the gitpixel shim can write temp/log files; codex also uses
# absolute paths to binaries (/usr/bin/zsh -c '/usr/bin/cat ...'), bypassing the
# PATH shim entirely, so codex discovery ops must be parsed from transcripts.
HARNESS_FLAGS = {
    "codex": ["codex", "exec", "--skip-git-repo-check", "--sandbox", "workspace-write"],
    "gemini": ["gemini", "--skip-trust", "--approval-mode", "plan", "-m", "gemini-2.5-flash", "-p"],
    "opencode": ["opencode", "run", "-m", "google/gemini-2.5-flash"],
}

# Environment variables per harness
HARNESS_ENV = {
    "codex": {},
...
def run_harness(harness, prompt, repo_path, shim_dir, shim_log_path):
    ...
    For codex with workspace-write sandbox, SHIM_LOG must be inside the workspace
    since codex restricts writes to the workspace dir. We use a temp log inside
    the repo and copy it to the final location after the run.
    """
    # Build environment
    env = os.environ.copy()
    ...
    # For codex workspace-write sandbox, SHIM_LOG must be inside the workspace
    actual_shim_log = shim_log_path
    if harness == "codex":
        actual_shim_log = Path(repo_path) / ".shim_log.tmp"
        env["SHIM_LOG"] = str(actual_shim_log)
        # Remove any stale log
        if actual_shim_log.exists():
            actual_shim_log.unlink()
    else:
    ...
    try:
        ...
        # Copy codex shim log from workspace to artifacts
        if harness == "codex" and actual_shim_log.exists():
            shutil.copy2(str(actual_shim_log), str(shim_log_path))
            actual_shim_log.unlink()

        return result.returncode, result.stdout, result.stderr, elapsed
    except subprocess.TimeoutExpired:
        elapsed = time.time() - start
        # Copy codex shim log even on timeout
        if harness == "codex" and actual_shim_log.exists():
            shutil.copy2(str(actual_shim_log), str(shim_log_path))
            actual_shim_log.unlink()
...
def run_single(task, arm, harness, run_id, artifacts_dir, repo_path, gitpixel_bin,
    ...
    result = {
        ...
        "drop_reason": None,
        "wall_seconds": None,
        "index_build_seconds": index_build_seconds if arm == "B" else None,
        "shim_log_path": None,
...
def main():
    ...
    for task in tasks:
        ...
        # Build index + graph for arm B (once per task)
        index_built = False
        index_build_seconds = None
        idx_log = ""
        if "B" in arms:
            print(f"  Building index + graph for arm B...", file=sys.stderr)
        ...
        with ThreadPoolExecutor(max_workers=min(args.max_concurrent, len(cells))) as executor:
            ...
            for arm, harness, run_id in cells:
                # Skip arm B if index build failed
                if arm == "B" and not index_built:
                    all_results.append({
                        "run_id": run_id, "task_id": task_id, "arm": arm,
