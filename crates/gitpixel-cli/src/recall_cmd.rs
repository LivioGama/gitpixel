...
use clap::Subcommand;
use gitpixel_recall::ingest::ingest_source;
use gitpixel_recall::model::format_ms;
use gitpixel_recall::search::{SearchFilters, search};
use gitpixel_recall::segment::SegmentSet;
use gitpixel_recall::sources::SourceAdapter;
use gitpixel_recall::sources::claude::ClaudeAdapter;
use gitpixel_recall::store::{RecallStore, SessionRow};
use serde_json::json;
...
fn run_daemon_cmd(cmd: RecallDaemonCmd) -> Result<(), String> {
    let root = gitpixel_recall::ensure_recall_dir().map_err(|e| format!("recall dir: {e}"))?;
    match cmd {
        RecallDaemonCmd::Start { foreground } => {
            if foreground {
                let service =
                    gitpixel_serve::RecallService::open().map_err(|e| e.to_string())?;
                return gitpixel_serve::daemon::run_corpus(service).map_err(|e| e.to_string());
            }
            if crate::daemon_ping(&root) {
                println!(
                    "recall daemon already running ({})",
                    gitpixel_serve::daemon::socket_path(&root).display()
                );
                return Ok(());
            }
...
fn try_recall_daemon(action: &str, params: serde_json::Value) -> Option<serde_json::Value> {
    let root = gitpixel_recall::recall_dir();
    let req = gitpixel_serve::api::Request::Recall {
...
/// Same convention as gitpixel-context::estimate_tokens (len/4, ceil).
...
fn run_context(
    ...
    let segments = SegmentSet::open(&gitpixel_recall::segments_dir())?;
    let vectors = gitpixel_recall::vector::VectorStore::open(&gitpixel_recall::vectors_dir())?;
    let now = now_ms();
...
fn run_ask(
    ...
    for g in &result.groups {
        println!("{}", gitpixel_recall::ask::format_group(g));
    }
    Ok(())
}
...
fn run_maxtest(
    ...
    for term in &terms {
        ...
        let (turns, sessions) =
            gitpixel_recall::search::count_matches(&store, &segments, &escaped, true, &filters)?;
...
fn open_store() -> Result<RecallStore, String> {
    gitpixel_recall::ensure_recall_dir().map_err(|e| format!("recall dir: {e}"))?;
    RecallStore::open(&gitpixel_recall::db_path()).map_err(|e| format!("recall.db: {e}"))
}

fn adapters(filter: Option<&str>) -> Result<Vec<Box<dyn SourceAdapter>>, String> {
    let all: Vec<Box<dyn SourceAdapter>> = vec![
        Box::new(ClaudeAdapter::new()),
        Box::new(gitpixel_recall::sources::codex::Adapter::new()),
        Box::new(gitpixel_recall::sources::cursor::Adapter::new()),
        Box::new(gitpixel_recall::sources::gemini::Adapter::new()),
        Box::new(gitpixel_recall::sources::opencode::Adapter::new()),
        Box::new(gitpixel_recall::sources::zcode::Adapter::new()),
        Box::new(gitpixel_recall::sources::devin::Adapter::new()),
    ];
    match filter {
        None => Ok(all),
        Some(csv) => {
...
fn run_index(source: Option<String>, stats: bool, full: bool) -> Result<(), String> {
    ...
    let mut segments = SegmentSet::open(&gitpixel_recall::segments_dir())?;
...
fn parse_time(spec: &str, now_ms: i64) -> Result<i64, String> {
    ...
    gitpixel_recall::model::parse_iso_ms(&full)
...
fn session_line(s: &SessionRow) -> String {
    ...
    let ts_note = match s.ts_source {
        gitpixel_recall::model::TsSource::Iso | gitpixel_recall::model::TsSource::UnixMs => {
            String::new()
        }
...
fn run_status(json: bool) -> Result<(), String> {
    ...
    if json {
        let out = json!({
            "location": gitpixel_recall::recall_dir(),
            "db_bytes": db_bytes,
    ...
    println!(
        "corpus: {} ({:.1} MB)",
        gitpixel_recall::recall_dir().display(),
