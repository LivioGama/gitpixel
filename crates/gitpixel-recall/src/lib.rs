//! gitpixel-recall — machine-wide LLM transcript retrieval corpus.
//!
...
/// Corpus root: `$GITPIXEL_RECALL_DIR`, else `~/.local/share/gitpixel/recall`.
...
pub fn recall_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("GITPIXEL_RECALL_DIR")
        && !dir.is_empty()
    {
