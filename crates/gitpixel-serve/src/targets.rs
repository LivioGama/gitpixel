...
//! `gitpixel-recall`'s hybrid channel fusion), and assigns P0/P1/P2 tiers.
...
use gitpixel_graph::split_ident_words;
use gitpixel_graph::targets::SymbolHit;
...
pub struct SignalInputs {
    ...
    pub envelope: Option<gitpixel_graph::Envelope>,
}
...
// RRF constants — mirrors gitpixel-recall/src/hybrid.rs, generalized to five
...
fn symbol_value(sym: &gitpixel_graph::SymbolRow) -> Value {
    json!({
        "uid": sym.uid,
        "name": sym.name,
...
pub fn compute_targets(
    ...
    let (lower_bound, unresolved, graph_state) = if inputs.graph_available {
        let env = inputs.envelope.clone().unwrap_or(gitpixel_graph::Envelope {
            lower_bound: false,
...
mod tests {
    use super::*;
    use gitpixel_graph::store::{SymbolKind, SymbolRow};
