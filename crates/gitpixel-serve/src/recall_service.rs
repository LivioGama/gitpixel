...
use gitpixel_recall::ask::{ask, format_group};
use gitpixel_recall::embed::{Embedder, open_default_embedder, run_backfill};
use gitpixel_recall::ingest::ingest_source;
use gitpixel_recall::search::{SearchFilters, format_hit, search};
use gitpixel_recall::segment::SegmentSet;
use gitpixel_recall::sources::SourceAdapter;
use gitpixel_recall::store::RecallStore;
use gitpixel_recall::vector::VectorStore;
...
impl RecallService {
    pub fn open() -> Result<Self, ServeError> {
        let root = gitpixel_recall::ensure_recall_dir()
            .map_err(|e| ServeError::Msg(format!("recall dir: {e}")))?;
        let store = RecallStore::open(&gitpixel_recall::db_path())
        ...
        match action {
            ...
            "ask" => {
                ...
                let filters: SearchFilters = params
                    ...
                    .unwrap_or_default();
                let segments = SegmentSet::open(&gitpixel_recall::segments_dir())?;
                let vectors = VectorStore::open(&gitpixel_recall::vectors_dir())?;
                if !lexical_only {
    ...
    fn refresh_agents(&mut self, agents: &std::collections::BTreeSet<&'static str>) {
        ...
        match SegmentSet::open(&gitpixel_recall::segments_dir()) {
            Ok(mut segments) => {
