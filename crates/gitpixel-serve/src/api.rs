...
use gitpixel_core::index::{MAX_FILE_BYTES, open_regular_bounded};
...
impl Service {
    ...
    fn op_context(&mut self, uid: &str, budget_tokens: Option<usize>) -> Result<Value, String> {
        ...
        use gitpixel_context::estimate_tokens;
...
mod bridge {
    ...
    pub fn is_fresh(root: &Path, db: &Path) -> bool {
        gitpixel_graph::build::is_fresh(root, db)
    }
    ...
    pub fn trace(store: &GraphStore, from_uid: &str, to_uid: &str) -> Result<Value, String> {
        gitpixel_graph::trace::trace(store, from_uid, to_uid, 8)
            .map(to_val)
            .map_err(es)
    }
    ...
    pub fn changes(store: &GraphStore, root: &Path, base: Option<&str>) -> Result<Value, String> {
        gitpixel_graph::changes::detect(store, root, base)
            .map(to_val)
            .map_err(es)
    }
    ...
    pub fn render_context(items: &[Item], budget_tokens: usize) -> (String, &'static str, usize) {
        use gitpixel_context::{ContextItem, Layer, fit_to_budget_detailed, render};
        ...
        if let Some((target, neighbors)) = mapped.split_first() {
            ...
            let target_tokens = gitpixel_context::estimate_tokens(&target_text);
...
mod tests {
    ...
    fn context_budget_covers_whole_response() {
        ...
        let tokens = gitpixel_context::estimate_tokens(&serialized);
        assert!(
            tokens <= 50,
        ...
        assert!(
            text.is_empty() || gitpixel_context::estimate_tokens(text) <= 50,
            "text should be empty or tiny when budget is 50, got {} tokens",
            gitpixel_context::estimate_tokens(text)
        );
