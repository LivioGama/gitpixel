...
//! (`GITPIXEL_SNIPER_STATE_ROOT` overrides the state root for tests).
...
/// State root: `GITPIXEL_SNIPER_STATE_ROOT` > `XDG_STATE_HOME` > `~/.local/state`.
pub fn resolve_state_root() -> PathBuf {
    if let Ok(root) = std::env::var("GITPIXEL_SNIPER_STATE_ROOT")
        && !root.is_empty()
    {
